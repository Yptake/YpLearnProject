...
public class FragmentLifecycleCallbacksImpl extends FragmentManager.FragmentLifecycleCallbacks {
    ...
    public void onFragmentStarted(@NotNull FragmentManager fm, Fragment f) {
        ...
        if (ArmsUtils.findViewByName(Objects.requireNonNull(f.getActivity()).getApplicationContext(), f.getActivity(), "public_toolbar") != null) {
            if (f.getActivity() instanceof AppCompatActivity) {
                ((AppCompatActivity) f.getActivity()).setSupportActionBar(ArmsUtils.findViewByName(f.getActivity().getApplicationContext(),
                        f.getActivity(), "public_toolbar"));
                Objects.requireNonNull(((AppCompatActivity) f.getActivity()).getSupportActionBar()).setDisplayShowTitleEnabled(false);
                setToolBar((AppCompatActivity) f.getActivity(), ArmsUtils.findViewByName(f.getActivity().getApplicationContext(),
                        f.getActivity(), "public_toolbar"));
            } else {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    f.getActivity().setActionBar(ArmsUtils.findViewByName(f.getActivity().getApplicationContext(),
                            f.getActivity(), "public_toolbar"));
                    Objects.requireNonNull(f.getActivity().getActionBar()).setDisplayShowTitleEnabled(false);
                }
            }
        }
        if (ArmsUtils.findViewByName(f.getActivity().getApplicationContext(), f.getActivity(), "public_toolbar_title") != null) {
            ((TextView) ArmsUtils.findViewByName(f.getActivity().getApplicationContext(), f.getActivity(), "public_toolbar_title"))
                    .setText(f.getActivity().getTitle());
        }
     ...
     * @param mToolbar
     */
    private void setToolBar(AppCompatActivity mActivity, Toolbar mToolbar) {
        mToolbar.setTitle("");
